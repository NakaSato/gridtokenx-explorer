...
describe('fetchResource', () => {
  ...
  it('should be called with proper arguments', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(uri, expect.anything());
  ...
  it('should throw exception upon exceeded size', async () => {
    ...
    await expect(fetchResource(uri, headers, 100, 100)).rejects.toThrowError('Max Content Size Exceeded');
  });
  ...
  it('should handle AbortSignal', async () => {
    ...
    await expect(fetchResource(uri, headers, 100, 100)).rejects.toThrowError('Gateway Timeout');
  });
  ...
  it('should handle unexpected result', async () => {
    // @ts-expect-error fetch does not have mocked fn
    (global.fetch as any).mockRejectedValueOnce({ data: 'unexpected exception' });
