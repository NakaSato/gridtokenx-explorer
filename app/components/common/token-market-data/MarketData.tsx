        ...
        >
            <div className="mb-1 flex items-center gap-2">
                <span
                    ...
                    className="overflow-hidden text-ellipsis whitespacnowrap"
                >
                    {label}
                </span>
                {(rank ?? 0) > 0 && (
                    <span className="whitespacnowrap rounded bg-[#1ED190] px-[5px] text-xs text-[#1C2120]">
                        Rank #{rank}
                    </span>
                )}
                {lastUpdatedAt && (
...
MarketData.Series = function MarketDataSeries({ data }: { data: MarketDataProps[] }) {
    return (
        <div className="flex flex-col gap-1 xs:flex-row sm:gap-2">
