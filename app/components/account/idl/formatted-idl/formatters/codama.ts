...
function isIxAccountNodePda(account: InstructionAccountNode): boolean {
  ...
  if (account.defaultValue?.kind === 'conditionalValueNode') {
    return (
      account.defaultValue.ifTrue?.kind === 'pdaValueNode' || account.defaultValue.ifFalse?.kind === 'pdaValueNode'
    );
  }
    ...
    const formattedIdl: FormattedIdl = {
      ...
      pdas: uniqPdaNodes?.map(pdaValueNode => {
        ...
        return {
          docs: linkedPda?.docs || [],
          name: pda.name,
          seeds: pda.kind === 'pdaLinkNode' ? (linkedPda ? getSeedsFromPda(linkedPda) : []) : getSeedsFromPda(pda),
        };
      }),
