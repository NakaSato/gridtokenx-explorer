...
export function AccountHeader({
  ...
  if (isToken && !isTokenInfoLoading) {
    if (isRedactedTokenAddress(address)) {
      return <TokenMintHeader address={address} mintInfo={mintInfo} parsedData={undefined} tokenInfo={undefined} />;
    }
...
function TokenMintHeaderCard({
  ...
  return (
    <div className="flex items-center">
      {unverified && (
        <div className="rounded-md border border-yellow-200 bg-yellow-50 px-4 py-3 text-yellow-900" role="alert">
          Warning! Token names and logos are not unique. This token may have spoofed its name and logo to look like
          another token. Verify the token&apos;s mint address to ensure it is correct. If you are the token creator,
          please verify your token on{' '}
          <a
