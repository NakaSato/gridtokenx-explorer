...
function SquadsProposalInspectorCard({ account, onClear }: { account: string; onClear: () => void }) {
  ...
  if (error || !vaultTransaction) {
    return <ErrorCard text={`Error loading vault transaction: ${error?.message}`} retry={onClear} retryText="Clear" />;
  }
  ...
  const convertVaultTransactionToMessage = (vaultTx: typeof VaultTransaction.prototype): VersionedMessage => {
    ...
    const solanaMessage = new MessageV0({
      ...
      header: {
        ...
        numReadonlyUnsignedAccounts: message.accountKeys.length - message.numSigners - message.numWritableNonSigners,
    ...
    <div className="container mt-4">
      ...
      {signature ? (
        ...
        isSquadsProposalAccountData(inspectorData) ? (
          ...
          <LoadedView transaction={inspectorData} onClear={reset} showTokenBalanceChanges={showTokenBalanceChanges} />
        )
      ) : (
          ...
          <tr>
            ...
            <td className="text-right">
              {message.staticAccountKeys.length === 0 ? (
                ...
                <AddressWithContext pubkey={message.staticAccountKeys[0]} validator={feePayerValidator} />
              )}
            </td>
          </tr>
        </TableCardBody>
