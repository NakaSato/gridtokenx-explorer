...
export function InstructionsSection({ message }: { message: VersionedMessage }) {
  ...
  for (let i = 0; i < hydratedTables.length; i++) {
    ...
    if (table && table[1] === FetchStatus.FetchFailed) {
      return (
        <ErrorCard
          text={`Failed to fetch address lookup table: ${message.addressTableLookups[i].accountKey.toString()}`}
        />
      );
    }
  }
...
function InspectorInstructionCard({
  ...
  if (anchorProgram.program) {
    return (
      <ErrorBoundary fallback={<UnknownDetailsCard key={index} index={index} ix={ix} programName="Unknown Program" />}>
        <AnchorDetailsCard
