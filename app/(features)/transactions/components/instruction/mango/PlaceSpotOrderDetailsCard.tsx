...
export function PlaceSpotOrderDetailsCard(props: {
  ...
  useEffect(() => {
    async function getOrderLotDetails() {
      ...
      const mangoSpotMarket = await getSpotMarketFromSpotMarketConfig(ix.programId, cluster.url, mangoSpotMarketConfig);
  ...
  return (
    ...
    >
      ...
      <tr>
        <td>Order type</td>
        <td className="lg:text-right">{info.orderType}</td>
      </tr>

      {info.clientId !== '0' && (
        <tr>
          <td>Client Id</td>
          <td className="lg:text-right">{info.clientId}</td>
        </tr>
      )}

      <tr>
        <td>Side</td>
        <td className="lg:text-right">{info.side}</td>
      </tr>
      ...
      {orderLotDetails !== null && (
        <tr>
          <td>Limit price</td>
          {/* todo fix price */}
          <td className="lg:text-right">{orderLotDetails?.price} USDC</td>
        </tr>
      )}
      ...
      {orderLotDetails !== null && (
        <tr>
          <td>Size</td>
          <td className="lg:text-right">{orderLotDetails?.size}</td>
        </tr>
      )}
    </InstructionCard>
  );
