  ...
  return (
    ...
    >
      ...
      {offsets.map((offset, index) => {
        const signature = extractData(tx, offset.signatureInstructionIndex, ix.data, offset.signatureOffset, 64);
              ...
              <td className="lg:text-right">
                {signature ? (
                  <Copyable text={Buffer.from(signature).toString('base64')}>
                    <span className="font-mono">{Buffer.from(signature).toString('base64')}</span>
                  </Copyable>
                ) : (
                  'Invalid reference'
                )}
              ...
              >
                {message ? (
                  <Copyable text={Buffer.from(message).toString('base64')}>
                    <span className="font-mono">{Buffer.from(message).toString('base64')}</span>
                  </Copyable>
                ) : (
                  'Invalid reference'
                )}
