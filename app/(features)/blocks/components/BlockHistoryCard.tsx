...
import { StatusBadge } from '@/app/(shared)/components/shared/StatusBadge';
...
import { pickClusterParams } from '@/app/(shared)/utils/url';
      ...
      <CardContent className="p-0">
                ...
                {filteredTransactions.slice(0, numDisplayed).map((tx, i) => {
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td>
                        {tx.costUnits !== undefined ? new Intl.NumberFormat('en-US').format(tx.costUnits) : 'Unknown'}
                      </td>
                      <td>
        ...
        {filteredTransactions.length > numDisplayed && (
          <div className="border-t px-6 py-4">
            <Button className="w-full" onClick={() => setNumDisplayed(displayed => displayed + PAGE_SIZE)}>
              Load More
            </Button>
          </div>
        )}
...
function FilterLink({
  ...
  return (
    <DropdownMenuItem asChild key={programId}>
      <Link href={href} className={programId === currentFilter ? 'font-semibold' : ''}>
