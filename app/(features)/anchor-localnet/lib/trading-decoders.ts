...
import { readU64LE, readI64LE, readU32LE, readU16LE } from './bytes';
...
export function decodeMarket(data: Uint8Array, address: string): MarketData {
  const d = data.subarray(8);
  return {
    address,
    authority: new PublicKey(d.subarray(0, 32)).toBase58(),
    totalVolume: Number(readU64LE(d, 32)),
    lastClearingPrice: Number(readU64LE(d, 48)),
    vwap: Number(readU64LE(d, 56)),
    activeOrders: readU32LE(d, 64),
    totalTrades: readU32LE(d, 68),
    marketFeeBps: readU16LE(d, 72),
    clearingEnabled: d[74] === 1,
    minPrice: Number(readU64LE(d, 80)),
    maxPrice: Number(readU64LE(d, 88)),
    buyDepthCount: 0,
    sellDepthCount: 0,
...
export function decodeOrder(data: Uint8Array, address: string): OrderData {
  const d = data.subarray(8);
  ...
  return {
    ...
    orderId: Number(readU64LE(d, 64)),
    amount: Number(readU64LE(d, 72)),
    filledAmount: Number(readU64LE(d, 80)),
    pricePerKwh: Number(readU64LE(d, 88)),
    orderType: ORDER_TYPES[orderType] ?? `Unknown(${orderType})`,
    status: ORDER_STATUSES[status] ?? `Unknown(${status})`,
    createdAt: Number(readI64LE(d, 104)),
    expiresAt: Number(readI64LE(d, 112)),
  };
}
...
function decodeDepth(d: Uint8Array, base: number, count: number): PriceLevelData[] {
  const levels: PriceLevelData[] = [];
  for (let i = 0; i < Math.min(count, MAX_DEPTH_LEVELS); i++) {
    const at = base + i * PRICE_LEVEL_SIZE;
    levels.push({
      orderCount: readU16LE(d, at + 16),
      price: Number(readU64LE(d, at)),
      totalAmount: Number(readU64LE(d, at + 8)),
    });
  }
  return levels;
}
...
export function decodeZoneMarket(data: Uint8Array, address: string): ZoneMarketData {
  const d = data.subarray(8);
  ...
  return {
    ...
    zoneId: readU32LE(d, 32),
    numShards: d[36],
    totalVolume: Number(readU64LE(d, 40)),
    activeOrders: readU32LE(d, 48),
    totalTrades: readU32LE(d, 52),
    lastClearingPrice: Number(readU64LE(d, 64)),
    capacity: Number(readU64LE(d, 72)),
    committedFlow: Number(readU64LE(d, 80)),
 ...
 * executed_at@160. */
export function decodeTradeRecord(data: Uint8Array, address: string): TradeData {
  const d = data.subarray(8);
  return {
    address,
    ...
    amount: Number(readU64LE(d, 128)),
    pricePerKwh: Number(readU64LE(d, 136)),
    totalValue: Number(readU64LE(d, 144)),
    feeAmount: Number(readU64LE(d, 152)),
    executedAt: Number(readI64LE(d, 160)),
  };
}
