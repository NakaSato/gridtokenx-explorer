...
function RenderEntry({ value, type }: { value: NeodymeSecurityTXT[keyof NeodymeSecurityTXT]; type: DisplayType }) {
  ...
  switch (type) {
    ...
    case DisplayType.Contacts:
      return (
        <td className="font-mono">
          <ul className="security-contacts lg:text-right [&.security-contacts]:text-left">
            {value?.split(',').map((c, i) => {
              const idx = c.indexOf(':');
              if (idx < 0) {
                //invalid contact
    ...
    case DisplayType.Auditors:
      ...
      return (
        <td>
          <ul className="security-txt-auditors lg:text-right [&.security-txt-auditors]:text-left">
